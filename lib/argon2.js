...
const resource = await externalResources.add( `softvisio-node/argon2/resources/napi-v3-${process.platform}-${process.arch}.node` ).check();

const { "hash": _hash } = require( resource.location + "/argon2.node" );
...
export default { defaults, limits, hash, needsRehash, verify, ...types };
