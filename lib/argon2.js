...
export async function verifyHash ( digest, password, { version, memoryCost, timeCost, parallelism, keyId } = {} ) {
    ...
    memoryCost = Number( parsed.params?.m || memoryCost || DEFAULT.memoryCost );
    if ( memoryCost < 1 || memoryCost > maxMemoryCost ) throw "memoryCost is not valid";

    timeCost = Number( parsed.params?.t || timeCost || DEFAULT.timeCost );
    if ( timeCost < 1 || timeCost > maxTimeCost ) throw "timeCost is not valid";

    parallelism = Number( parsed.params?.p || parallelism || DEFAULT.parallelism );
    if ( parallelism < 1 || parallelism > maxParallelism ) throw "parallelism is not valid";
...
export function needsRehash ( digest, { id, version, memoryCost, timeCost, parallelism } = {} ) {
    ...
    return id !== parsed.id || Number( version ) !== parsed.version || Number( memoryCost ) !== Number( parsed.params?.m ) || Number( timeCost ) !== Number( parsed.params?.t ) || Number( parallelism ) !== Number( parsed.params?.p );
}
